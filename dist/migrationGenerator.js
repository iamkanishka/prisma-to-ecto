...
// Public entry point
// ---------------------------------------------------------------------------
function generateMigrationFiles(schema, outputDir) {
    ensureDir(outputDir);
    ...
    views.forEach((view, index) => {
        ...
        const tableName = (0, utils_1.toTableName)(view.name, view.tableName);
...
// Migration renderer
// ---------------------------------------------------------------------------
function renderMigration(model, tableName, schema) {
    const pkField = model.fields.find((f) => f.isId);
    const pkColumnDef = renderPkColumn(model, pkField);
    const columnFields = model.fields.filter((f) => f.isScalar && !f.isId && !f.isUpdatedAt && !f.isForeignKey);
    ...
    const fieldUniqueLines = columnFields
        .filter((f) => f.isUnique)
        .map((f) => {
        const col = (0, utils_1.toSnakeCase)(f.columnName ?? f.name);
        return `    create unique_index(:${tableName}, [:${col}])`;
    });
    // Fulltext indexes
    const fulltextLines = model.fulltextIndexes.map((idx) => {
        const cols = idx.fields.map(utils_1.toSnakeCase).map((c) => `:${c}`).join(", ");
    ...
    const tableOpts = [];
    if (isCompositePk || isUuidPk)
        tableOpts.push("primary_key: false");
    // @@schema → prefix: opt
    if (model.dbSchema)
        tableOpts.push(`prefix: "${model.dbSchema}"`);
...
function renderIndexLine(tableName, idx, unique) {
    const cols = idx.fields.map(utils_1.toSnakeCase).map((c) => `:${c}`).join(", ");
    const fn_ = unique ? "unique_index" : "index";
    const opts = [];
    if (idx.options.name)
        opts.push(`name: "${idx.options.name}"`);
    if (idx.options.where)
        opts.push(`where: "${idx.options.where}"`);
    if (idx.options.type) {
...
// PK column
// ---------------------------------------------------------------------------
function renderPkColumn(model, pkField) {
    if (!pkField)
        return null;
...
function renderColumn(field) {
    ...
    if (field.type.startsWith("Ecto.Enum:")) {
        ...
        type = `:${(0, utils_1.nativeTypeToMigration)(field.nativeType, field.type)}`;
    }
    else {
        type = `:${(0, utils_1.prismaTypeToMigration)(field.type)}`;
    }
    const opts = columnOpts(field);
    return `      add :${col}, ${type}${opts.length ? `, ${opts.join(", ")}` : ""}`;
}
function columnOpts(field) {
    const opts = [];
    opts.push(field.isOptional ? "null: true" : "null: false");
    // Size from @db.VarChar(n)
    if (field.nativeType) {
        const size = (0, utils_1.nativeTypeSize)(field.nativeType);
        if (size)
            opts.push(`size: ${size}`);
    }
    if (field.default) {
        const migDefault = (0, utils_1.prismaDefaultToMigration)(field.default, field.type);
        if (migDefault !== undefined)
            opts.push(`default: ${migDefault}`);
    }
    return opts;
}
function renderForeignKey(field, schema) {
    const rel = field.relation;
    const relatedModel = schema.models.find((m) => m.name === rel.relatedModel);
    const refTable = relatedModel
        ? (0, utils_1.toTableName)(relatedModel.name, relatedModel.tableName)
        : (0, utils_1.toTableName)(rel.relatedModel);
    const fkColName = rel.fields && rel.fields.length > 0
        ? (0, utils_1.toSnakeCase)(rel.fields[0])
        : (0, utils_1.toSnakeCase)(`${field.name}_id`);
    const relPkField = relatedModel?.fields.find((f) => f.isId);
    const refType = relPkField?.idKind === "uuid" || relPkField?.idKind === "auto" ? ":uuid"
        : relPkField?.idKind === "cuid" || relPkField?.idKind === "string" ? ":string"
            : undefined;
    const refTypeOpt = refType ? `, type: ${refType}` : "";
    // Referential action
    const onDelete = rel.onDelete ? (0, utils_1.referentialActionToEcto)(rel.onDelete) : "nothing";
    const onUpdate = rel.onUpdate ? (0, utils_1.referentialActionToEcto)(rel.onUpdate) : undefined;
...
// View migration stub
// ---------------------------------------------------------------------------
function renderViewMigration(model, tableName) {
    const cols = model.fields
        .filter((f) => f.isScalar)
        .map((f) => `--   ${(0, utils_1.toSnakeCase)(f.columnName ?? f.name)} ${(0, utils_1.prismaTypeToMigration)(f.type)}`)
        .join("\n");
...
function collectImplicitJoinTables(models) {
    const seen = new Set();
    const joinTables = [];
    const modelSet = new Set(models.map((m) => m.name));
    for (const model of models) {
        for (const field of model.fields) {
            if (field.relation?.kind === "one-to-many" &&
                ...
                if (!other)
                    continue;
                const otherSide = other.fields.find((f) => f.relation?.relatedModel === model.name &&
                    f.isArray &&
                    !f.relation?.isOwner &&
                    !f.relation?.fields);
                if (!otherSide)
                ...
                joinTables.push({
                    tableName: `${(0, utils_1.toSnakeCase)(nameA)}_${(0, utils_1.toSnakeCase)(nameB)}`,
                    modelA: nameA, tableA: (0, utils_1.toTableName)(nameA, modelAObj.tableName),
                    schemaA: modelAObj.dbSchema,
                    modelB: nameB, tableB: (0, utils_1.toTableName)(nameB, modelBObj.tableName),
...
function renderJoinTableMigration(jt, schema) {
    const modName = `${jt.modelA}${jt.modelB}`;
    const refTypeA = jt.pkKindA === "uuid" || jt.pkKindA === "auto" ? ", type: :uuid"
        : jt.pkKindA === "cuid" || jt.pkKindA === "string" ? ", type: :string"
            : "";
    const refTypeB = jt.pkKindB === "uuid" || jt.pkKindB === "auto" ? ", type: :uuid"
...
defmodule MyApp.Repo.Migrations.Create${modName} do
  ...
  def change do
    create table(:${jt.tableName}, primary_key: false) do
      add :${(0, utils_1.toSnakeCase)(jt.modelA)}_id, references(:${jt.tableA}, on_delete: :delete_all${refTypeA}${prefixA}), null: false
      add :${(0, utils_1.toSnakeCase)(jt.modelB)}_id, references(:${jt.tableB}, on_delete: :delete_all${refTypeB}${prefixB}), null: false
    end

    create unique_index(:${jt.tableName}, [:${(0, utils_1.toSnakeCase)(jt.modelA)}_id, :${(0, utils_1.toSnakeCase)(jt.modelB)}_id])
  end
end
`);
}
