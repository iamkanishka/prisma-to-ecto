...
function main() {
  ...
  if (opts.generateSchemas)   console.log(`  Schemas →  ${opts.schemaOut}`);
  if (opts.generateMigrations) console.log(`  Migrations → ${opts.migrationOut}`);
  console.log();

  const schema = parsePrismaSchema(opts.schemaPath);

  console.log(
    `\x1b[36mParsed ${schema.models.length} model(s), ${schema.enums.length} enum(s)\x1b[0m\n`
  );

  if (opts.generateSchemas) {
    console.log("Generating Ecto schemas...");
    generateEctoSchema(schema, opts.schemaOut);
    console.log();
  }

  if (opts.generateMigrations) {
    console.log("Generating migrations...");
    generateMigrationFiles(schema, opts.migrationOut);
    console.log();
  }
